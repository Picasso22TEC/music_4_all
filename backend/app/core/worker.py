"""
Worker de descargas — consume la cola Redis y procesa jobs de forma asíncrona.

Flujo:
  1. BRPOP de la cola music4all:queue:downloads
  2. prepare() → obtiene tracks desde la API de Tidal
  3. download_single_track() en thread → callback publica progreso via Pub/Sub
  4. Persiste en PostgreSQL vía HistoryRepository
  5. Actualiza job state en Redis
"""
import asyncio
import json
import logging
...
async def start_worker(
    ...
    """Loop principal del worker. Arranca como asyncio.Task en el lifespan."""
...
async def _process_job(
    ...
    for i, track in enumerate(tracks):
        def make_cb(idx: int):
            def cb(p: float) -> None:
                ...
                # Publicar desde thread sync al event loop async
        ...
        try:
            ...
            logger.error("Track download error: %s", exc)
    ...
    if done == total:
        ...
        await _update_state(redis, job_id, title, DownloadJobStatus.FAILED, done / total * 100,
                            error="Una o más pistas fallaron")
...
async def _update_state(
    ...
    state = {
        ...
        "error": error, "file_path": file_path,
    }
