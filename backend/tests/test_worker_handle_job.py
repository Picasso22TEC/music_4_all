...
async def test_handle_job_uses_owner_engine_and_releases(registry, monkeypatch):
    engine = MagicMock()
    ...
    await worker._handle_job(job, registry, MagicMock(), MagicMock(), MagicMock())
...
async def test_handle_job_fails_when_no_engine(registry, monkeypatch):
...
async def test_handle_job_fails_when_no_user_id(registry, monkeypatch):
    process = AsyncMock()
    update = AsyncMock()
    monkeypatch.setattr(worker, "_process_job", process)
    monkeypatch.setattr(worker, "_update_state", update)
