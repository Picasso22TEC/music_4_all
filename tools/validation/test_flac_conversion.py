...
def main():
    ...
    if not flac_files:
        print("⚠️ No hay archivos .flac descargados")
        return
    ...
    for flac_file in flac_files:
        ...
        if status == "valid_flac":
            ...
            print(f"   ⏱️ {result['duration_seconds']:.1f}s / {result['size_bytes'] / 1024 / 1024:.1f}MB")
            valid_count += 1
        else:
            invalid_count += 1
    ...
    if invalid_count > 0:
        print("⚠️ ATENCIÓN: Hay archivos que NO son FLAC válidos. Revisa la configuración de FFMPEG.")
