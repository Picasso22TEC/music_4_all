...
- **Descripción**: 3 de 141 tests fallan: `TestDownloadError::test_invalid_track_id` y `TestDownloadError::test_invalid_job_id` (ambos `KeyError: 'engine'` — `app.state.engine` no inicializado en el fixture del test) en `tests/integration/test_download_flow.py`; `TestCleanup::test_pubsub_unsubscribed_on_disconnect` en `tests/test_ws_downloads.py` (posible race condition, ver `docs/troubleshooting.md` #4.1). El job `test-backend` usa `pytest tests/ -v --tb=short || echo "No tests found — skipping"`, por lo que **estas 3 fallas no rompen el pipeline**.
- **Evidencia**: `docs/roadmap.md` §2.3; `.github/workflows/ci.yml` línea ~121.
- **Impacto técnico**: el patrón `|| echo` es excesivamente permisivo — fue diseñado para el caso "no hay tests", pero también enmascara fallos reales. Cualquier regresión futura en `tests/` pasará CI en verde.
- **Impacto de negocio**: riesgo de desplegar regresiones no detectadas; falsa sensación de seguridad en `main`.
- **Recomendación**: (1) corregir el fixture de `test_download_flow.py` para inicializar `app.state.engine` (causa raíz simple — fixture incompleto, no bug de producción); (2) investigar la race condition de `test_pubsub_unsubscribed_on_disconnect` (ver `docs/troubleshooting.md` #4.1); (3) **una vez los 3 tests pasen**, cambiar `|| echo "No tests found — skipping"` por una condición que solo aplique cuando `pytest` reporta "no tests collected" (código de salida 5), y bloquear en cualquier otro fallo.
- **Esfuerzo estimado**: S (fixture fix: horas) + M (race condition WS: 1-2 días de investigación).
- **Prioridad**: **P0** (el cambio de CI para que bloquee de verdad es la pieza de mayor apalancamiento — sin esto, cualquier otra mejora de testing puede degradarse silenciosamente).
- **Severidad**: **High**.
